...
class CNN(nn.Module):
    def __init__(self, height: int, width: int, channels: int, class_count: int):
        ...
        self.initialise_layer(self.conv2b)

        self.max_pool2b = nn.MaxPool2d(
            kernel_size=(2, 2),
        ...
        self.initialise_layer(self.conv3b)

        self.max_pool3b = nn.MaxPool2d(
            kernel_size=(2, 2),
        ...
        self.initialise_layer(self.conv4b)

        self.max_pool4b = nn.MaxPool2d(
            kernel_size=(5, 1),
           
        )

        # First fully connected layer
        self.full_connect1 = nn.Linear(5520, 200)
        ...
        self.linear = nn.Linear(40, 20)

    def BiRnn(self, audios: torch.Tensor) -> torch.Tensor:
        y = self.max_pool1(audios)
        y = torch.flatten(y, start_dim=1)
        rnn = nn.GRU(input_size=1600,hidden_size=200,num_layers=1,dropout=0.25,bidirectional=True)
        y,h=rnn(y)
        return y
    ...
    def forward(self, audios: torch.Tensor) -> torch.Tensor:
        ...
        x = self.max_pool2(x)
        #print("x shape after 2 is",x.shape)
        x = F.relu(self.conv3(x))
        x = self.max_pool3(x)
        #print("x shape after 3 is",x.shape)
        x = F.relu(self.conv4(x))
        x = self.max_pool4(x)
        #print("x shape after 4 is",x.shape)
        ...
        xb = self.max_pool2b(xb)
        xb = F.relu(self.conv3b(xb))
        xb = self.max_pool3b(xb)
        xb = F.relu(self.conv4b(xb))
        xb = self.max_pool4b(xb)
        ...
        x = torch.cat((x,xb),1)
        x = torch.cat((x,self.BiRnn(audios)),1)
        #print (x.shape)
